...
# Define file names and URLs for all platforms
...
mkdir -p "../../checkmarx-ast-teamcity-plugin-agent/src/runner/bin/2.0.0"
...
cp ./tmp/cx.exe ./checkmarx-ast-teamcity-plugin-agent/src/runner/bin/2.0.0/cx.exe
chmod +x ./checkmarx-ast-teamcity-plugin-agent/src/runner/bin/2.0.0/cx.exe
rm -r tmp/*()
rm  ${filename_windows}
...
cp ./tmp/cx ./checkmarx-ast-teamcity-plugin-agent/src/runner/bin/2.0.0/cx
rm -r tmp/*
rm  ${filename_linux}
...
cp ./tmp/cx ./checkmarx-ast-teamcity-plugin-agent/src/runner/bin/2.0.0/cx-mac
rm -r tmp
rm ${filename_darwin}
